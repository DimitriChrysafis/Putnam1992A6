...
def pointGenerator(num_points=4):
    ...
    """
    np.linalg.norm(points, axis=1): This calculates the Euclidean norm (magnitude) 
    of each point along axis 1, making in an array of shape (num_points,) containing 
    the norms of each point.
    
    points /= ...: Divides each point by its norm. Makeing sure they are on the sphere
    """
    return points
...
def isInCenter(points):
    ...
    #the way this works is actually kind of smart
    # calculate the signs of the distances from the center to the planes
    # return True if all signs are <= 0 or >= 0, meaning the center is inside the convex hull
...
def compileSize(max_size_bytes=1000000000):
    ...
    # divives work from cpu cores
    # it maps the simulate_single function to each core.
